...
class ResNet25D_column(nn.Module):
    def __init__(
        self,
        block: Type[Union[BasicBlock, Bottleneck]],
        layers: List[int],
        ...
        super(ResNet25D_column, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
...
class ResNet25D(nn.Module):
    def __init__(
        ...
        self.fc = self.fc = nn.Linear(512 * 3 * block.expansion, num_classes)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # See note [TorchScript super()]
        x0 = self.column0(x[:, :, 0, :, :].float())
        x1 = self.column1(x[:, :, 1, :, :].float())
        x2 = self.column2(x[:, :, 2, :, :].float())
